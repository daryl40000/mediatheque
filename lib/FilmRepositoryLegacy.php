<?php
/**
 * Ancien moteur dvdthèque (table `films`) — repli si le schéma catalogue
 * (bibliotheque) n’est pas encore présent.
 *
 * Les helpers d’affichage sont dans FilmPresentation ; cette classe ne doit
 * plus croître pour du formatage.
 */
...
final class FilmRepositoryLegacy
{
    ...
    public function distinctStyles(): array
    {
        ...
        $styles = [];
        foreach ($rows as $row) {
            foreach (FilmPresentation::splitStyles((string) $row['styles']) as $style) {
                $styles[$style] = true;
            }
        }
