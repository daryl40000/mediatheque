<?php
/**
 * Icône poubelle : retirer un numéro des envies ou de la collection.
 *
...
$isWishlistPage = $pageStatut === Moncine\LibraryStatut::WISHLIST;
$isPossessed = Moncine\MagazineSupport::isPossessed($issue);

// Envies : toujours. Collection : numéros possédés (papier ou PDF).
if (!$isWishlistPage && !$isPossessed) {
    return;
}

$confirmMsg = $isWishlistPage
    ? 'Retirer le n° ' . $numeroLabel . ' de vos envies ?'
    : 'Retirer le n° ' . $numeroLabel . ' de vos magazines ?';
$title = $isWishlistPage ? 'Retirer des envies' : 'Retirer de la collection';
?>
<form method="post"
