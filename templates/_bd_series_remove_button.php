<?php
/**
 * Retirer une série BD de la collection ou des envies.
 *
...
$buttonLabel = $isWishlist ? 'Retirer de mes envies' : 'Retirer de mes BD';
$title = $isWishlist
    ? 'Retirer cette série de vos envies'
    : 'Retirer cette série de vos BD';

$confirmParts = [
    $isWishlist
        ? 'Retirer « ' . $seriesTitre . ' » de vos envies ?'
        : 'Retirer « ' . $seriesTitre . ' » de vos BD ?',
];
if ($tomeCount > 0) {
    $confirmParts[] = $tomeCount . ' tome(s) seront retirés de votre bibliothèque.';
}
$confirmParts[] = 'Le catalogue partagé (fiches série et tomes) ne sera pas supprimé.';
$confirmMsg = implode("\n", $confirmParts);
?>
...
<section class="magazine-series-remove-panel film-delete-panel">
    ...
    <p class="hint">
        La série disparaîtra de votre liste<?= $isWishlist ? ' d’envies' : '' ?>.
        Vous pourrez la rajouter plus tard depuis le catalogue.
    </p>
    <form method="post"
          action="/traiter-serie-bd.php"
          class="inline-form"
